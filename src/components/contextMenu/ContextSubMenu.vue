<template>
  <div
    ...
    :style="{
      ...
      zIndex: zIndex,
      left: `${position.x + (options.leftPosition || 0)}px`,
      top: `${position.y + 4}px`,
    }"
    ...
    >
      <slot>
        <template v-for="(item, i) in items" :key="i">
          ...
          <!--Custom render-->
          <VNodeRender
            v-if="item.divided && globalHasSlot('separatorRender')"
            :vnode="vnode"
          />
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const globalIsFullScreenContainer = inject(
      "globalIsFullScreenContainer"
    ) as boolean;
    //#endregion
    ...
    function setAndFocusNotDisableItem(isDown: boolean, startIndex?: number) {
      if (isDown) {
        ...
        for (
          let i = startIndex !== undefined ? startIndex : menuItems.length - 1;
          i >= 0;
          i--
        ) {
          if (!menuItems[i].isDisabledOrHidden()) {
    ...
    function setAndFocusCurrentMenu(index?: number) {
      ...
      if (index !== undefined)
        currentItem =
          menuItems[Math.max(0, Math.min(index, menuItems.length - 1))];
    ...
    const thisMenuInsContext: SubMenuContext = {
      ...
      closeCurrentSubMenu: () =>
        thisMenuContext.getParentContext()?.closeOtherSubMenu(),
      moveCurrentItemFirst: () => setAndFocusNotDisableItem(true),
      moveCurrentItemLast: () => setAndFocusNotDisableItem(false),
      moveCurrentItemDown: () =>
        setAndFocusNotDisableItem(
          true,
          currentItem ? menuItems.indexOf(currentItem) + 1 : 0
        ),
      moveCurrentItemUp: () =>
        setAndFocusNotDisableItem(
          false,
    ...
    const thisMenuContext: SubMenuParentContext = {
      ...
      getParentAbsY: () =>
        menu.value ? getTop(menu.value, parentContext.container) : 0,
      ...
      nextTick(() => {
        ...
        if (adjustPosition.value && menuEl && scroll.value) {
          ...
          scrollHeight.value =
            menuEl.offsetHeight -
            avliableHeight +
            fillPadding * 2; /* Padding */
          overflow.value = yOverflow > 0;

          if (xOverflow > 0)
            //X overflow
            position.value.x -=
              (getParentWidth ? getParentWidth() : 0) +
              menuEl.offsetWidth -
              fillPadding;
