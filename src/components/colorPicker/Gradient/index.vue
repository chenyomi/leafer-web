...
-->
<template>
  <GradientControls v-if="!solidColor" :type="state.gradientType" :activeObject="canvas.activeObject.value" :change-gradient-control="changeGradientControl" />
...
const getChangeData = () => ({
  points: state.gradientType === 'color' ? [state.gradientPoints[state.activePointIndex]] : state.gradientPoints,
  type: state.gradientType,
  degree: state.colorDegree
...
const changeActivePointIndex = (index: number) => {
  // 初次进去的时候存在document的活跃对象是input，然后input存在 各种事件，因此执行顺序会影响正常逻辑
...
const updateGradientLeft = (left: number, index: number, actionName: 'onStartChange' | 'onChange' | 'onEndChange' = 'onChange') => {
