<template>
  <div
    :class="[
      ...
      disabled ? statusClassNames.disabled : '',
    ]"
  >
      ...
      <div v-if="colorModes.includes(mode)">
        ...
        <div :class="[`${baseClassName}__sliders-wrapper`]">
          ...
          <template v-if="showPrimaryColorPreview">
            <div
              :class="[
                `${baseClassName}__sliders-preview`,
                `${baseClassName}--bg-alpha`,
              ]"
            >
              <span
                :class="[`${baseClassName}__sliders-preview-inner`]"
                :style="{
                  background: isGradientLinear
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const mode = ref<TdColorModes>(
      props.colorModes?.length === 1 ? props.colorModes[0] : 'monochrome'
    );
    ...
    const updateColor = () =>
      color.value.update(innerValue.value || defaultEmptyColor.value);

    const formatModel = ref<TdColorPickerProps['format']>(
      color.value.isGradient ? 'CSS' : 'RGB'
    );
    ...
    const formatValue = () => {
      ...
      // 如果当前格式是 RGB，使用 RGBA 格式
      const format = props.format === 'RGB' ? 'RGBA' : props.format;
      const formatColor = colorMap[format] || color.value.rgba;
      return formatColor;
    };
    ...
    const addRecentlyUsedColor = () => {
      if (
        recentlyUsedColors.value === null ||
        recentlyUsedColors.value === false
      ) {
        return;
      }
      const colors = cloneDeep(recentlyUsedColors.value as string[]) || [];
      const currentColor = color.value.isGradient
        ? color.value.linearGradient
        : color.value.rgba;
    ...
    const emitColorChange = (trigger?: ColorPickerChangeTrigger) => {
      const newColor = formatValue();
      console.log('emitColorChange newColor:', newColor);
    ...
    watch(
      ...
      (newColor) => {
        if (newColor !== formatValue()) {
          updateColor();
          mode.value = color.value.isLinerGradient
            ? 'linear-gradient'
            : color.value.isRadialGradient
            ? 'radial-gradient'
            : 'monochrome';
        }
      }
    ...
    const handleModeChange = (value: TdColorModes) => {
      ...
      if (value === 'linear-gradient') {
        color.value.update(
          color.value.gradientColors.length > 0
            ? color.value.linearGradient
            : DEFAULT_LINEAR_GRADIENT
        );
     ...
     * @returns
     */
    const handleFormatModeChange = (format: TdColorPickerProps['format']) =>
      (formatModel.value = format);
    ...
    const handleSatAndValueChange = ({
      ...
      let changeTrigger: ColorPickerChangeTrigger =
