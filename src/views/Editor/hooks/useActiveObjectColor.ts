...
import colorImg from '@/assets/images/color.jpg'
 ...
 * @param coords
 */
const convertCoordsToDeg = ({ x1, y1, x2, y2 }: LinearGradientCoords<number>) => (Math.atan2(y2 - y1, x2 - x1) * 180) / Math.PI + 90
...
export function useColor(
  ...
  const colorBlock = computed(() => (index: number) => {
    ...
    if (!color.value || !Array.isArray(color.value) || index >= color.value.length) {
      return {
    ...
    if (isString(colorVal)) {
      ...
      background += `,${colorVal.stops.map((cs: any) => `${cs.color} ${cs.offset * 100}%`).join(',')})`
    ...
    return {
      class: 'w18px h18px rd-4px',
      style: {
        background: background || `url(${colorImg})`,
        borderRadius: '4px',
        backgroundSize: '100% 100%',
        backgroundRepeat: 'no-repeat',
        backgroundPosition: 'center'
  ...
  const openColorPicker = (index: number) => {
    ...
    appInstance.editor.service.invokeFunction((accessor) => {
      ...
      let active = null
      if (canvas.activeObject.value.tag == 'Box') {
        if (canvas.activeObject.value.findOne('ShapeLine')) {
          active = canvas.activeObject.value.findOne('ShapeLine')
        } else if (canvas.activeObject.value.findOne('ShapeRect')) {
          active = canvas.activeObject.value.findOne('ShapeRect')
        } else if (canvas.activeObject.value.findOne('ShapePolygon')) {
          active = canvas.activeObject.value.findOne('ShapePolygon')
        } else if (canvas.activeObject.value.findOne('ShapeStar')) {
          active = canvas.activeObject.value.findOne('ShapeStar')
        } else if (canvas.activeObject.value.findOne('ShapeEllipse')) {
          active = canvas.activeObject.value.findOne('ShapeEllipse')
        } else {
          active = canvas.activeObject.value
        }
      } else if(canvas.activeObject.value.tag == 'Group' && canvas.activeObject.value.name === 'Text') {
        active = canvas.activeObject.value.findOne('Text')
      } else {
        active = canvas.activeObject.value
      }
      ...
      // 计算初始颜色：优先使用当前索引颜色，其次使用第一个颜色。纯色用其 color，渐变用第一个 stop 的 color
      const current = Array.isArray(color.value) ? (color.value[index] || color.value[0]) : undefined
      const initialColor = (() => {
        if (!current) return undefined
        if (isString(current)) return current as string
        if (current?.type === 'solid') return current.color
        if (typeUtil.isGradient(current)) return current?.stops?.[0]?.color
        return undefined
      })()
  ...
  return {
     ...
     */
    changeOpacity(value: number | undefined, customHandler?: (color: GColor) => void) {
