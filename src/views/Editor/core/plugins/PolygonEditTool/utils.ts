...
/**
 * 计算多边形允许的最大圆角半径
 * 输入是扁平化的 path 数组，返回数值
 */
export function getMaxCornerRadiusFromPolygon(flatData: number[]): number {
  const points: { x: number; y: number }[] = []

  // 遍历pathData，每3个数提取x,y点，忽略命令数字
  for (let i = 0; i < flatData.length - 1; i += 3) {
  ...
  for (let i = 0; i < points.length; i++) {
    ...
    const b = Math.hypot(curr.x - next.x, curr.y - next.y)
    const c = Math.hypot(next.x - prev.x, next.y - prev.y)

    // 利用余弦定理计算夹角余弦值和夹角弧度
    const cosTheta = (a * a + b * b - c * c) / (2 * a * b)
    const theta = Math.acos(Math.max(-1, Math.min(1, cosTheta))) // 限制数值范围，防止误差

    // 计算半角的正切值，计算内切圆的最大半径
    const tanHalfTheta = Math.tan(theta / 2)

    // 最大圆角半径，不超过相邻边的一半长度限制
    const maxRadius = (Math.min(a, b) * tanHalfTheta) / (1 + tanHalfTheta)

    // 记录所有顶点中最小的最大圆角半径，确保不会越界
