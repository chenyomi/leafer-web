  ...
  <div class="flex items-center justify-between middle-menu">
    ...
    <div class="middle-menu-item flex items-center">
      <div class="flex items-center" @click="textOpen = !textOpen" ref="textOpenRef">
        <ali-icon type="icon-test2" :size="20" style="color: #6b6b6b" :class="{ 'text-active': (textOpen || activeTool === 'text') }"></ali-icon>
        <icon-down style="color: #6b6b6b; margin-left: 0px" :size="10" :class="{ 'text-active': textOpen }" />
      </div>
    ...
    <div class="middle-menu-item flex items-center">
      <div class="flex items-center current-active" @click="graphicOpen = !graphicOpen" ref="graphicOpenRef">
        <ali-icon
          ...
          :size="20"
          style="color: #6b6b6b"
          :class="{ 'text-active': graphicOpen || isShape }"
        ></ali-icon>
        <icon-down style="color: #6b6b6b; margin-left: 0px" :size="10" :class="{ 'text-active': graphicOpen || activeTool === 'graphic' }" />
      </div>
      <div class="menu-box graphic-box" v-if="graphicOpen">
        <template v-for="item in uniquegGraphic" :key="item.json.name">
          <div v-if="item.name" class="graphic-box-name">{{ item.name }}</div>
    ...
    <div class="middle-menu-item flex items-center cursor-pointer">
      <div class="flex items-center" @click="shapeChange" ref="shapeOpenRef">
        <ali-icon type="icon-xianduan" :size="22" style="color: #6b6b6b" :class="{ 'text-active': (shapeOpen || activeTool === 'line' || isShapeLine) }"></ali-icon>
        <icon-down style="color: #6b6b6b; margin-left: 0px" :size="10" :class="{ 'text-active': shapeOpen }" />
      </div>
    ...
    <div class="middle-menu-item flex items-center">
      <div class="flex items-center" @click="penChange" ref="penOpenRef">
        <ali-icon type="icon-mingcute_pen-line" :size="20" :class="{ 'text-active': (penOpen || activeTool === 'penDrawToLine') }"></ali-icon>
        <icon-down style="color: #6b6b6b; margin-left: 0px" :size="10" :class="{ 'text-active': penOpen }" />
      </div>
    ...
    <div class="middle-menu-item flex items-center">
      <div class="flex items-center" @click="handleCommentOpen" ref="commentOpenRef">
        <ali-icon type="icon-msg" :size="24" style="color: #6b6b6b" :class="{ 'text-active': commentOpen }" />
      </div>
    </div>
  </div>
</template>
...
const isShape = computed(() => {
  return activeTool.value === 'graphic' && !['Bracket', "Circular"].includes(graphicType.value.tag)
})
const isShapeLine = computed(() => {
  return activeTool.value === 'graphic' && ['Bracket', "Circular"].includes(graphicType.value.tag)
})
