...
const handleSearch = (value: string): void => {
  ...
  if (value.trim()) {
    ...
    if (originalCategoriesStack.length > 0) {
      ...
      breadcrumbItems.value = ['components', ...breadcrumbNames, `搜索: ${value}`]
    } else {
      // 如果在根层级搜索，显示：components → 搜索: 关键词
      breadcrumbItems.value = ['components', `搜索: ${value}`]
    }
  } else {
    // 无搜索关键词时，根据当前分类层级显示面包屑
    if (categoriesStack.value.length > 0) {
      const breadcrumbNames = categoriesStack.value.map(cat => cat.categoryName)
      breadcrumbItems.value = ['components', ...breadcrumbNames]
    } else {
      breadcrumbItems.value = ['components']
    }
  }
...
// 当前面包屑路径
const breadcrumbItems = ref(['components']) // 初始只显示一级
...
/**
 * 面包屑导航到指定层级
 * @param {number} id - 面包屑索引（0 为根 components）
 * @returns {Promise<void>} 无返回
 */
