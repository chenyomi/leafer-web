 ...
 * 获得树节点数据
 */
const getTreeData = (objects: any, searchKey?: string, parentVisible = true): ITreeNodeData[] => {
  const objs: ITreeNodeData[] = []
  if (!objects) return objs
  for (const object of objects) {
    // 跳过ID为modified-svg的元素，这是SVG图层高亮预览时创建的临时元素
    if (object.id === 'modified-svg') {
      continue
    }

    const isCollection = typeUtil.isCollection(object)
    const proxyData = object.proxyData
    const children = isCollection ? getTreeData(object.children, searchKey, parentVisible && proxyData.visible) : []
 ...
 * 移动节点
 */
const moveNode = (data: { e: DragEvent; dragObject: IUI; dropObject: IUI; dropPosition: DropPosition; excludeGroupObject: boolean }) => {
...
const updateSelectedkeys = async () => {
  ...
  if (treeRef.value && treeRef.value.virtualListRef) {
    ...
    const nodeRect = document.querySelector(`.arco-tree-node[data-key='${selectedkeys.value[0]}']`)?.getBoundingClientRect()
    // TODO 数据太多时经常白屏不展示、无法混动？
    // 判断是否在可视区域外
    if (!nodeRect || containerRect.top - nodeRect.top > nodeRect.height || nodeRect.top - containerRect.top > treeHeight.value) {
...
const onInputChange = (value: string, e: Event) => {
  ...
  // treeRef.value?.updateTreeView()  saveState!
}
</script>
...
<template>
  <div ref="splitRef" class="h-[calc(100vh-90px)]">
    <div style="padding: 0 10px">
      <a-input-search placeholder="Search..." style="margin-bottom: 8px" v-model="searchKey" />
    </div>
    <Tree
      ref="treeRef"
      size="small"
