        ...
        <template #userId="{ column, record }">
          <template v-if="column.key === 'userId'">
            <div class="id-item flex items-center">
              ...
              <div class="id-item-id">OEditor-{{ record.code }}</div>
            </div>
          </template>
        </template>
  ...
  <a-modal v-model:visible="isSuccessModalOpen" :footer="false" :closable="false" @cancel="handleSuccessClose" width="480px">
    ...
    <div class="modal-content mt20px" style="padding: 0">
      <div class="modal-content-item flex items-center">
        <div class="modal-list-item-label">岗位：</div>
        <div class="modal-list-item-value">外部编辑</div>
      </div>
  ...
  <a-modal v-model:visible="isInfoModalOpen" :footer="false" :closable="false" @cancel="handleInfoClose" width="580px">
    ...
    <div class="modal-content mt20px">
      <div class="modal-content-item flex items-center mt20px">
        ...
        <div class="modal-list-item-value">OEditor-{{ editor.code }}</div>
      </div>
      ...
      <div class="modal-content-item flex mt10px">
        <div class="modal-list-item-label" style="flex-shrink: 0">负责类别：</div>
  ...
  const processCategoryData = (data: any[]): any[] => {
    ...
    return sortedData.map(item => {
      ...
      const processedItem = {
        ...item,
        title: titleText,
        type: item.categoryType, // 添加type属性用于CSS选择
...
/* 分类类型样式 */
.arco-tree-node-title[data-attr-type="1"] {
  color: #3883e9;
  font-weight: 500;
}

.arco-tree-node-title[data-attr-type="2"] {
  color: #63a44f;
  font-weight: 500;
}

.arco-tree-node-title[data-attr-type="4"] {
  color: #b23232;
  font-weight: 500;
}

.arco-tree-node-title[data-attr-type] {
  font-weight: 500;
}
.exception {
  color: #b23232;
