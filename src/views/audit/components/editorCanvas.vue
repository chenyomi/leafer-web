<template>
  <div class="editor-box" style="height: 100%">
        ...
        <transition name="preview-zoom">
          ...
          >
            ...
            <div class="preview-content">
              <div class="comment-text" v-html="`${textToEmojiHtml(comment.commentBody)}`"></div>
...
import svg from '@/assets/svg/89.svg'
import { canvasSizeAttrs, Image } from 'leafer-ui'
import { useRoute } from 'vue-router'
import { textToEmojiHtml, emojiHtmlToText } from '@/utils/emoji'
import { useClickBlank } from '@/hooks/useClickBlank'

const route = useRoute()
const id = route.query.outId as string //获取素材id
const type = route.query.type as string //获取素材类型
const dashboardId = route.query.boardOutId as string //获取画板id
 ...
 * @param y Y坐标
 */
const saveCommentPositionToCache = (commentId: string, x: number, y: number) => {
  const cacheKey = `comment_position_${currentPageInfo.value.outId}`
  const existingCache = JSON.parse(localStorage.getItem(cacheKey) || '{}')
  existingCache[commentId] = { x, y, timestamp: Date.now() }
  localStorage.setItem(cacheKey, JSON.stringify(existingCache))
  console.log(`缓存评论位置: ${commentId} -> (${x}, ${y})`)
}
 ...
 * @returns 位置信息或null
 */
const getCommentPositionFromCache = (commentId: string) => {
  const cacheKey = `comment_position_${currentPageInfo.value.outId}`
  const cache = JSON.parse(localStorage.getItem(cacheKey) || '{}')
  return cache[commentId] || null
}

/**
 * 本地存储工具函数 - 清理指定评论的位置缓存
 * @param commentId 评论ID
 */
const clearCommentPositionCache = (commentId: string) => {
  const cacheKey = `comment_position_${currentPageInfo.value.outId}`
  const cache = JSON.parse(localStorage.getItem(cacheKey) || '{}')
  if (cache[commentId]) {
    delete cache[commentId]
    localStorage.setItem(cacheKey, JSON.stringify(cache))
    console.log(`清理评论位置缓存: ${commentId}`)
  }
}

/**
 * 本地存储工具函数 - 应用缓存的评论位置到评论列表
 * @param comments 评论列表
 */
const applyCommentPositionCache = (comments: any[]) => {
  const cacheKey = `comment_position_${currentPageInfo.value.outId}`
  const cache = JSON.parse(localStorage.getItem(cacheKey) || '{}')

  comments.forEach((comment) => {
    const cachedPosition = cache[comment.commentId]
    if (cachedPosition) {
      comment.x = cachedPosition.x
      comment.y = cachedPosition.y
...
const getCommentList = async () => {
  console.log(currentPageInfo.value.boardOutId, currentPageInfo.value.outId)
  ...
  // 应用本地缓存的评论位置
  applyCommentPositionCache(commentList.value)
...
const getCommentDetail = async (commentId: any) => {
  ...
  if (rootComment) {
    ...
    if (cachedPosition) {
      ...
      console.log(`编辑端评论详情应用缓存位置: ${rootComment.commentId} -> (${cachedPosition.x}, ${cachedPosition.y})`)
    }
...
onMounted(async () => {
  ...
  //监听切换list后重新调用评论
  emitter.on('changeMaterial', (data: any) => {
    currentPageInfo.value.boardOutId = data.boardOutId
    currentPageInfo.value.outId = data.pageOutId
    currentPageInfo.value.materialId = data.outId
    getCommentList()
  })
  // 监听评论列表刷新事件
  ...
  console.log('screenWidth', screenWidth, canvas.app.tree.width)
  // canvas.app.tree.width = screenWidth - 400 //设置画布初始化宽度
  // canvas.app.tree.y = 100 //设置画布初始化Y坐标
  canvas.ruler.enabled = false
  canvas.app.config.zoom.min = 0.8 //最小缩放比例为0.5
  canvas.app.config.zoom.max = 5 //最大缩放比例为5

  // const canvasActiveObj = canvas.getActiveObject()
  // canvas.app.editor.config.disabled = true
  // canvas.app.editor.hittable = false  //禁用编辑器交互
  // canvas.app.editor.editable = false
  // canvas.app.editor.disabled = false
  // canvasActiveObj.draggable = false
...
/**
 * 计算聚合后的锚点列表
 * - 缩放取值优先使用数值型 zoom.value，其次回退到实时 canvas.app.tree.scale，最终回退 1
 */
const clusteredCommentList = computed(() => {
  if (!commentList.value || commentList.value.length === 0) return []
  const currentScale = typeof zoom.value === 'number' && !Number.isNaN(zoom.value) ? zoom.value : Number(useEditor()?.canvas?.app?.tree?.scale) || 1
...
onMounted(() => {
  ...
  if (canvas && canvas.app) {
    // 监听缩放事件：同步缩放值并刷新锚点位置
    canvas.app.on('zoom', () => {
      // 先同步缩放到本地 zoom.value，避免使用旧值计算导致偏移
      const currentScale = Number(canvas?.app?.tree?.scale) || 1
      zoom.value = currentScale
      // 广播刷新事件（携带缩放值），让其他组件保持一致
      emitter.emit('refresh-anchors', currentScale)
      // 延迟一帧确保缩放值已更新，再刷新锚点位置
      requestAnimationFrame(() => {
        updateAnchorsPosition()
      })
    })
...
/**
 * 计算评论组件X坐标位置（与 Y 同步的缩放与偏移逻辑）
 * @param {number} pos - 原始X坐标（内容坐标系）
 * @returns {number} - 当前视图下的X坐标
 */
const calculateCommentX = (pos: number): number => {
  try {
    const { canvas } = useEditor()
    if (!canvas || !canvas.app) return pos

    // 缩放优先使用 zoom.value，其次回退到实时树缩放，最终回退 1
    const scale = typeof zoom.value === 'number' && !Number.isNaN(zoom.value) ? zoom.value : Number(canvas?.app?.tree?.scale) || 1

    // 偏移量回退 0，使用可选链避免首帧非响应值
    const offsetX = Number(canvas?.app?.tree?.x) || 0

    // 计算并进行 8 位小数精度处理，减少浮点误差引起的抖动
    const result = pos * scale + offsetX
    const fixed = Math.round(result * 100000000) / 100000000
    // console.log('x坐标位置:', fixed)
    return fixed
  } catch (error) {
...
const openComment = async (item: any) => {
  ...
  if (commentData.value && commentData.value.commentId == item.commentId) {
    ...
    const rootComment = info.find((item: any) => item.root == 0)
    const otherComment = info.filter((item: any) => item.root != 0).reverse()
    console.log(rootComment, otherComment, res)
    if (rootComment) {
      // 应用本地缓存的位置信息到根评论
      const cachedPosition = getCommentPositionFromCache(rootComment.commentId)
      if (cachedPosition) {
        rootComment.x = cachedPosition.x
        rootComment.y = cachedPosition.y
        console.log(`编辑端点击锚点时应用缓存位置: ${rootComment.commentId} -> (${cachedPosition.x}, ${cachedPosition.y})`)
      }
 ...
 */
const handleCommentEnter = async (comment: any, event?: MouseEvent) => {
  // 清除离开定时器
