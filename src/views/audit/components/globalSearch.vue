<template>
  ...
  <div class="materials-grid">
    <!-- 数据加载时显示loading -->
    <LoadingOverlay :visible="loading && categoryMaterials.length === 0" />
          ...
          <div class="material-card-wrapper">
            ...
            <template v-if="item.status == 0">
              ...
              <div
                class="update-tag"
                v-if="
                  item.editorUser.length > 0 &&
                  (item.editorUser.some((k: any) => k.userId.includes(userId)) ||
                    item.editorUser.some((k: any) => k.roles && k.roles.some((r: any) => r.roleKey === 'OAAdmin')))
                "
              >
                更新
              </div>
            ...
            <div
              class="material-card"
              :class="{ selected: selectedItems.includes(item.outId), disabled: item.currentUser && item.currentUser.userId !== userId }"
              @click.stop="handleMaterialClick(item)"
            >
              <!-- 正在编辑中标识 -->
...
watch(
  ...
  (newKeyword) => {
    if (newKeyword && typeof newKeyword === 'string') {
      console.log('检测到keyword变化:', newKeyword)
      params.value.name = newKeyword
      countParams.value.keyword = newKeyword
      // 当关键词变化时，自动执行搜索
      handleSearch(newKeyword)
    }
  },
  { immediate: false } // 初始值在组件挂载时已经处理，这里不需要立即执行
)
...
const getMaterialList = async () => {
  ...
  try {
    ...
    if (params.value.pageNum === 1) {
      //   categoryMaterials.value = sortMaterials(res.records)
      categoryMaterials.value = res.records
    } else {
      //   categoryMaterials.value = sortMaterials([...categoryMaterials.value, ...res.records])
      categoryMaterials.value = [...categoryMaterials.value, ...res.records]
    }
...
onMounted(() => {
  ...
  // 监听全局搜索事件
  emitter.on('globalSearch', handleGlobalSearchEvent)
  ...
  try {
    const savedStatus = localStorage.getItem('globalSearchActiveStatus')
    if (savedStatus !== null) {
  ...
  // 优先使用从父组件传入的keyword prop
  if (props.keyword && typeof props.keyword === 'string') {
    params.value.name = props.keyword
    countParams.value.keyword = props.keyword
    console.log('使用传入的搜索关键词:', props.keyword)
  } else {
    // 如果没有传入keyword，则尝试从localStorage加载
    try {
      const savedSearchKeyword = localStorage.getItem('globalSearchKeyword')
      if (savedSearchKeyword !== null && savedSearchKeyword !== '') {
  ...
  // 注意：不再强制重置搜索关键词，保留从localStorage恢复的值
  // 只有在不是从详情页返回且没有保存的搜索关键词时才重置状态
  if (!fromDetailPage && !localStorage.getItem('globalSearchKeyword')) {
    params.value.status = ''
    activeTab.value = 'All'
  }
  ...
  if (fromDetailPage) {
    try {
      // 恢复选中的类型
      const savedType = localStorage.getItem('globalSearchActiveType')
      if (savedType && savedType.trim() !== '') {
        try {
          ...
          console.warn('解析保存的类型失败:', parseError)
        }
      }

      // 恢复搜索关键词name
      const savedSearchKeyword = localStorage.getItem('globalSearchKeyword')
      if (savedSearchKeyword && savedSearchKeyword.trim() !== '') {
        try {
          const parsedKeyword = JSON.parse(savedSearchKeyword)
          if (parsedKeyword) {
            params.value.name = parsedKeyword
          ...
          console.warn('解析保存的搜索关键词失败:', parseError)
        }
      }
      ...
      if (savedStatus && savedStatus.trim() !== '') {
        try {
          ...
          console.warn('解析保存的状态失败:', parseError)
        }
      }
...
onUnmounted(() => {
  ...
  // 移除全局搜索事件监听
  emitter.off('globalSearch', handleGlobalSearchEvent)

  console.log('globalSearchActiveType', activeType.value)
  // 保存当前选中的类型到localStorage，用于返回时恢复
  localStorage.setItem('globalSearchActiveType', JSON.stringify(activeType.value))

  // 保存当前选中的状态到localStorage，用于返回时恢复
  console.log('globalSearchActiveStatus', activeTab.value)
  localStorage.setItem('globalSearchActiveStatus', JSON.stringify(activeTab.value))

  // 保存当前搜索关键词到localStorage，用于返回时恢复
  console.log('materialEditorSearchKeyword', params.value.name)
  if (params.value.name) {
    localStorage.setItem('globalSearchKeyword', JSON.stringify(params.value.name))
  }
})
 ...
 * 处理全局搜索事件
 */
const handleGlobalSearchEvent = (keyword: string) => {
  console.log('Received global search event with keyword:', keyword)
  // 更新搜索参数
  params.value.name = (keyword ?? '').toString().trim() || undefined
  countParams.value.keyword = (keyword ?? '').toString().trim() || undefined
  // 同时更新localStorage以保持缓存同步
  if (params.value.name) {
    localStorage.setItem('globalSearchKeyword', JSON.stringify(params.value.name))
  } else {
